...
import { getBuiltInPhraseById, getPhrasePrompt } from '../data/phrases';
import { getPracticeLanguage } from '../lib/storage';
import { ensureDefaultPhraseCards, getPhraseCards, getSelectedDeckId } from '../lib/v11Storage';
...
export function PhrasePracticeScreen({ sourceClipId, onBack }: Props) {
  ...
  const [practiceLanguage, setPracticeLanguage] = useState<PracticeLanguage>('pt');

  const load = useCallback(async () => {
    setLoading(true);
    const deckId = await getSelectedDeckId();
    await ensureDefaultPhraseCards(deckId);
    const [phraseCards, language] = await Promise.all([
      getPhraseCards(deckId, sourceClipId),
      getPracticeLanguage(),
    ]);
    setCards(phraseCards.sort((a, b) => a.createdAt - b.createdAt));
  ...
  const promptText = useMemo(() => {
    if (!current) return '';
    if (!current.phraseId) return current.front;
    const phrase = getBuiltInPhraseById(current.phraseId);
    if (!phrase) return current.front;
    return getPhrasePrompt(phrase, practiceLanguage);
  }, [current, practiceLanguage]);
  ...
  if (loading) {
    return (
      <View style={styles.centered}>
        <Text style={styles.title}>Loading phrase cards...</Text>
      </View>
    );
  }
  ...
  if (cards.length === 0) {
    return (
      <View style={styles.centered}>
        <Text style={styles.title}>No phrases yet</Text>
        <Text style={styles.subtitle}>No phrases available for this selection.</Text>
  ...
  if (isDone) {
    return (
      <View style={styles.centered}>
        <Text style={styles.title}>Phrase session complete</Text>
